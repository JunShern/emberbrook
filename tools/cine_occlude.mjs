    ...
    while (sp) {
      ...
      if (n.count) {
        for (let k = n.start; k < n.start + n.count; k++) {
          ...
          if (t > 1e-4 && t < tmax && t < best) { best = t; if (anyHit) return best; }
        }
  ...
  return {
    ...
    nearField(posMap, aimMap, fovDeg, aspect) {
      ...
      for (const [sx, sy] of BUNDLE) {
        ...
        const t = cast(o, [dr[0] / L, dr[1] / L, dr[2] / L], standoff * 4, false) / standoff;
        hits.push({ndc: [sx, sy], t});
      ...
      return {frac: nearest === Infinity ? Infinity : +nearest.toFixed(3),
              ndc: nearestNdc, standoff: +standoff.toFixed(2),
