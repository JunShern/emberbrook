...
# the five, in the coordinator's order (the dominant blocker first)
FIVE = ["veg_gate_rimclump_11", "veg_gate_rimclump_1", "veg_gate_rimclump_2",
...
# --- restore the pristine geometry so a re-run is a recompute, not a second lift
restored = 0
for name in FIVE:
...
for name in FIVE:
    o = bpy.data.objects[name]
    Mx = o.matrix_world
    leaf_slots = {i for i, m in enumerate(o.data.materials) if m and m.name == LEAFMAT}
    assert leaf_slots, "%s carries no %s slot" % (name, LEAFMAT)
