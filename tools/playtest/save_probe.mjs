...
(async () => {
  ...
  const tap = async (name) => {
    const k = KEYS[name];
    await cdp.send('Input.dispatchKeyEvent', { type: 'keyDown', key: k.key, code: k.code, windowsVirtualKeyCode: k.vk, nativeVirtualKeyCode: k.vk });
    await sleep(60);
  ...
  const ready = async (budget = 90000) => {
    ...
    while (Date.now() - t0 < budget) {
      const v = await ev(`(()=>{try{ if(!window.SIM||!window.GS) return 0; const p=SIM.pos(); return (p&&isFinite(p.x))?1:0 }catch(e){return 0}})()`);
      if (v === 1) { await sleep(1200); return true; }
      await sleep(400);
    }
    return false;
  };
  ...
  console.log('§1 THE PLAYER WALKS SOMEWHERE, so the save has something of its own to carry');
  const before0 = await state();
  await hold('down', WALK_MS); await hold('right', WALK_MS);
  ...
  const menu = await ev(`(()=>{const v=[...document.querySelectorAll('.ebui-veil')].filter(e=>getComputedStyle(e).display!=='none').pop();
    if(!v) return null; return JSON.stringify([...v.querySelectorAll('.ebui-row')].map(r=>r.textContent.replace(/\\s+/g,' ').trim()))})()`);
  ...
  if (saveIdx >= 0) {
    const curIdx = rows.findIndex((_, i) => false); // the cursor starts at row 0 (menu.js opens on the first entry)
    for (let i = 0; i < saveIdx - Math.max(curIdx, 0); i++) await tap('down');
