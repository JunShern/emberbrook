...
| 4 | **shelf shop row pinches** [32–36.5, 19, −8.5..−6] | geometry (narrow pinch) | 34 steps / 5 runs + 17 steps; drive W→E stalls at [34.64, 19.07, −8.31] | CLOSED 2026-08-06 (Bet 2 it.8-10 window, 49260f0+): RECLASSIFIED, neither widen nor demote — the flanking-wall cells are the dead edge strip beside the weapon-shop gable; the open corridor beside them is 5-6 m wide (fill plan), the routed line keeps 1.5 m off the gable, drives 2/2 both ways. A building face at a lane edge is not a pinch; the recorded stall was pre-it.2 bytes. |
...
| 8 | **deep stairs ascent** [38.3, 2.3, −25.1] up | geometry (one-way, known) | 28 steps / 2 runs; eff 9% in shot; r21 left uphill 6/40 deliberately (descent-only + cut) | EXECUTED 2026-08-06 (Bet 2 it.10): the searched 3-leg w2.0 zigzag drives 13/13 BOTH WAYS (was 3/10 down, 2/10 up); wp1 [43.5,24.7,9.2] / wp2 [34,24.7,4.9] (deepstairs_search.py, 2448 candidates); "pivots":"asym" knob added after the balanced pivot 2 roofed its own landing; foot+fishdock ONE 663-cell component; the fish-dock__winch-foot boardwalk re-routed at grade past the foot. Earlier decision record: SIMPLIFY, not retire — retiring orphans the 1,446-cell fishdock component (its only other link is the blocked-by-design ladder), and the flight measured DEAD BOTH WAYS on the current bundle (built-line drives 3/10 down, 2/10 up — r21's 40/40 descent no longer holds). Replacement: 3-leg <=0.55-grade zigzag w2.0 under the width-scaled v2 split; first candidate screened against a 67k-tri art BVH and REJECTED (head deck plate in-tread 75 hits; wv_hut_weave-north_0 at x>=43.3; wf_ art excluded as re-derivable). Execution = next window's ring search + waterfront_build carry. |
...
aims across the lip). BLOCKER NAMED 2026-08-06 (Bet 2 it.7, current bundle): **cx_rail — the moorage parapet cx_build lays on the flight's own bar lines — holds 6 cells ON walk_e_weave-huts__moorage_l0 t04/t05's own ground at [73.4–74.4, 4.14–4.52, −26.8]**; both hairpin drives still stall 0.8–0.88 m short. Fix shape: chop-pattern carrier (three in the repo) with a cx_build re-run contract, or fix rail_on_blockouts' corridor guard — it clears the post's own tread, not the crossing flight's cells. FIXED 2026-08-06 (it.8-9, commit 49260f0 + the it.10 window): guard grew BODY_SAMPLES/on_walk_ribbon (the grid left unsampled strips on rotated treads — lip_diag.py), lf_lantern_1 reseated full-height (new carrier), and the l1/l2 pivot itself was SEARCHED to [70.1,29.4,4.1] at width 2.0 — the switchback drives 11/11 BOTH ways, tenant lane 3/3 both ways first time.
