...
### THE HEADLINE: three instruments disagreed about a staircase, and only the keys were right

`ch2.lockfive` leaves the player on the moorage deck at y ~1.0 and asks them to go to supper
at the Keepers' Cottage, up on the weave tier at y ~7.9. Three separate instruments were
pointed at "can you get up there", and **they gave three different answers.**

| instrument | answer | why it said that |
|---|---|---|
| `_court_probe --comp` (seeds: Maren, the `lockfive>weave` seam, box x 66..82 / z -30..-22) | **NO** — 182 and 179 cells sharing **exactly ONE** 0.4 m cell | and `--at` on that cell shows floors at **1.02 and 5.60 — a 4.58 m gap**. The shared cell is the artifact the tool's own legend warns of: "the fill settles from the neighbour's height, so membership is not symmetric and two components may overlap without joining" |
| `reach_probe` (maren -> weave stand, maren -> cottage door) | **YES**, `ok=true ... via edges [...]` | it LEAKED through that same artifact cell onto the tier and then legitimately took the cut. A fill that settles from a neighbour's height can manufacture a 4.58 m staircase out of nothing |
| `_court_probe --way` (the straight-line drive) | **NO** — stalls at `[76.91, 1.06, -26.24]` going up, `[77.35, 5.60, -26.59]` coming down | **and this is the one that nearly published a wrong headline.** Re-run along the agent's OWN trajectory it still stalls on leg 1, three metres short of a point the agent had walked through. A pusher that only tries `(dx,dz)`, `(dx,0)`, `(0,dz)` cannot negotiate a switchback deck |
| **the run, on real keys** | **YES** | deck -> `[71.45, 3.30, -27.88]` (an intermediate tier at y 3.3 that no fill above ever stood on) -> `[77.36, 5.60, -26.12]` -> the `weave` shot, at step 24 |

**SO THE WORLD IS OPEN AND THE DEFECT IS FINDABILITY: 22 of the run's first 24 steps went
into finding a staircase that was there the whole time.** The correction is recorded at this
length on purpose — the first draft of this section said "Lock Five is a basin" and would have
sent round 25 to BUILD a flight that already exists. CLAUDE.md's documentation bar, earned
again: an interpretation may be recorded only alongside the instrument that proved it, and
here the only competent instrument was the keyboard.

A further reach measurement, now correctly downgraded to "the fills say so, and the fills are
unreliable on this geometry": moorage-spawn -> quay-west and fishdock-lane -> quay-west both
return `no-path` at 1467 cells with a 1.6 m step gap at `[38.4, 12.47] -> [38.4, 14.07]`.
Treat that as a LEAD for a second one-way lip in the west, not as a finding.
...
1. **Make the climb out of Lock Five READ.** It is not missing, it is invisible: 22 steps to
   find it, and the frame shows painted ladders offering a way up the walk network does not
   carry. This is `findability_test`'s question asked about a staircase instead of a villager.
2. **Name what moved the body to y -3.90**, then fence or fill the weave tier's east hole
   (no floor at all in x 74.1..77.1 / z -23.9..-24.5). `walkStep` provably refuses to walk
   off that lip, so the mechanism is something else and guessing it is not allowed.
3. **DO NOT ask a flood fill whether Dellhollow's tiers connect.** `--comp`, `reach_probe`
   and `--way` were all wrong here in different directions. Until an instrument exists that
...
## Round 28 — 2026-08-05 · THE ARROW IS A PROMISE ABOUT THE GROUND

Leg 12's order: diagnose the COMMON root of PT-044 (moorage, arrow 5.0 m overhead) and
PT-045/046 (cottage, arrow 2.6 m down across a 1.5 m hole) before fixing either, then the
receipts, then the full NEW GAME run.

### §1 Both worlds are innocent, and so are both bands

The order named three candidates. Two are refuted by the engine's own walk network
(`_court_probe --grid ... "walk":true`, which prints `v` for a cell that has a visible
floor and NO walk network under it):

**The cottage (x 87..94, z −18.5..−23.5, y 6.5..11.5).** At x 88.58 the spur (`M`, y 10.8
down to 10.3, z −19) and the bridge (`l`, y 7.55, z −22.4) are separated by `#`/`v` for the
whole of z −20.0..−21.5. **The only continuous walk corridor between the two tiers is
x 92.0..93.75** — the `l` column that runs unbroken from z −20.8 to −22.5. That is exactly
where `dellhollow.map.json` joins them: the `lockhead→keepers-cottage` path and the
`weave-huts→keepers-cottage` bridge BOTH end at the landmark (92.61, 7.83, −22), and the
landmark's own retirement note already says so in words — *"The way down is cottage → weave
bridge → the moorage flight."* **The map draws no lane at x 88.6, so there is none:
option (b), a missing walk ribbon, is refuted.**

**And the seam is not across a void.** `cottage>crossing`'s band [88.579, 7.55, −22.391] is
`l` — walk floor, not blocked — squarely on the bridge, and it is the first point of
`routes.json`'s own `cottage:weave-huts__keepers-cottage@0.812..1` polyline. The moorage
band [75.011, 6.273, −25.2] is `M` on the upper switchback leg, likewise. **Option (c), a
band placed across a void, is refuted.** Both bands are right. Both destinations are right.
Both routed EDGES are right — from `lockfive` the only way toward the cottage is
`lockfive>weave`, and from `cottage` the metre-shortest of two is `cottage>crossing`, which
is the route the map's own note prescribes.

### §2 THE COMMON ROOT

> **The hint points at the SEAM. The seam is the END of the walk, not the NEXT STEP of it.**

`routeTo` costs its hops in CROW-FLIES metres (`gap()`, a 3-D straight line from the body to
each seam's band) and `hintTick` then decorates that seam's own marker. Both halves assume
the straight line between the body and the seam is GROUND. On a town built in tiers it
routinely is not — the walk network between them REVERSES (the moorage switchback) or
DETOURS (a spur that joins the bridge four metres east). Measured at the two failures:

| | straight line | walked, on the town's own route |
|---|---|---|
| PT-044 moorage `[76.15, 1.32, −27.11]` → `lockfive>weave` | **5.43 m** | **11.64 m** |
| PT-045 cottage `[90.22, 9.15, −20.36]` → `cottage>crossing` | **3.06 m** | **8.46 m** |

Rounds 11–19 taught the arrow to name the right destination (round 11), to lose to no rival
(round 18) and to sit on the ground it names rather than 97 px above it (round 19). All three
are about the arrow's PLACE ON THE SCREEN. This one is the first about the GROUND BETWEEN,
and it is the same sentence one level up: **the arrow is in the right place in the world and
the wrong place in the walk.**

### §3 The answer was already in the repo

`<town>.routes.json` — generated by `tools/routes_derive.mjs` from the map's OWN typed walk
edges — carries, per shot, the walked polyline between every entry and every exit. `lockfive`
carries the whole switchback (9 vertices, foot to seam); `cottage` carries the spur and the
bridge, welded at their shared endpoint (92.61, 7.83, −22). **Nothing solves a path here: it
READS THE AUTHORED ONE**, the same discipline followers.js keeps by sampling a trail instead
of pathfinding. `story_runtime` gets the file through `ROUTES.load()`, which already resolves
the town from the scene graph's own provenance, so this generalises to Emberbrook with no
new wiring.

**THE FIX (`public/js/story_runtime.js`).** `wayAim(edge, pos)` builds the shot's routes into
one point graph (consecutive vertices joined; vertices from different polylines welded at
0.35 m — the weld is what makes "the spur" and "the bridge" one walk through the cottage;
`role: "blocked"` polylines are excluded, since routes.json's own word for those is a way on
that looks walkable and ships no walk ribbon), Dijkstras from the vertex nearest the body to
the vertex nearest the seam, and returns **the first vertex at least 1.5 m ahead**.
`clampLift` grew an `aim` argument and translates markersTick's own div (the `translate`
property on its children — round 19's mechanism, unchanged) onto that point.

### §3b AND THE GATE ON IT IS THE ENGINE, NOT A NUMBER — which took two wrong numbers first

The first cut fired the aim whenever the walked route exceeded **`1.35 x`** the straight line
plus a metre. Both failures cleared it easily (11.64 vs 5.43 m; 8.46 vs 3.06 m) and it looked
finished. **Swept offline over both towns' routes files it fires on 238 of 878 (vertex, exit)
pairs in EMBERBROOK** — a town with no tiers — because two lanes that fork 1.5 m apart make a
3x "detour" on flat open road. Worked example, `emb-cine|arch`: a body on the orchard lane at
`[58.56, 0.05, −9.38]` would have been aimed BACK to `[60, 0, −8]`, 132° away from its own
seam, to reach a lane 1.1 m to its east. **Sending the one labelled arrow backwards down a
road the player can simply cross is a worse defect than the one being fixed**, and Chapter One
is played on that road.

The obvious repair was the mandate's own sentence — *not reachable from the tier it is drawn
on* — as a threshold: `|dy to the seam| > STEP_UP + STEP_DN` (1.43 m). It takes Emberbrook to
**0** fires and Dellhollow to 131, all on real tier crossings. **And it loses PT-046**, whose
stand `[90.22, 8.68, −21.69]` is 1.13 m above its seam and still closed 0.00 m of ten legs at
it. A body part-way down a ramp is not on a tier at all.

So neither number is the question, and the answer is the one `walk_engine_gate` and
`_court_probe` were each built to insist on: **ASK THE ENGINE.** `lineIsGround(me, seam)`
marches the straight line at 0.4 m through `SIM.walkFloors` under walkGround's own step window
(`+STEP_UP` / `−STEP_DN`, with its four 0.18 m plank-crack retries) and asks whether a body
could carry its feet along it and ARRIVE on the seam's own tier. Under the moorage deck it
marches at y≈1.3 and arrives 4.3 m below a seam at 6.27; across the cottage pit it finds a
column with no walk floor at all — which is exactly what PT-045 measured with `SIM.blocked`
**null**: nothing blocks it, there is nothing to step on.

**IT IS A FLOOR TEST AND NOT A BODY TEST, deliberately.** Adding `SIM.blocked` would make the
march refuse a doorway jamb the walker slides past, and detouring the arrow around a door the
player can walk through is the same class of harm as the 1.35 ratio. Both defects this round
fixes are missing FLOOR.

**The step rule is REPLICATED, and that is a smell round 27 §3 already named**: play3d exports
`walkFloors` but not `walkGround`, so four lines of story_runtime are a copy of a function the
game runs. `SIM.walkGround` belongs on the export list — a coordinator edit, reported and not
applied — and the constants are named in one place here so a change to play3d's has somewhere
to land.

**RULE 1 IS AMENDED ON PURPOSE: IT DECORATES, AND IT MAY AIM.** The label still lives inside
markersTick's `data-edge` div and still inherits every gate that layer applies — sealed,
denied, camFrom, frustum, UILOCK — so it remains structurally incapable of naming a way the
game is not offering. It FAILS CLOSED at seven places: `SIM.walkFloors` absent, the
straight line walkable, no routes file, no vertex within 4 m of the body, none within 2 m of
the seam, no path, an aim inside the 1.5 m lead, or an aim that projects behind the camera or
outside ±0.94 NDC. Any of them and the arrow is the seam again.
`?noaim=1` is the A/B; `Story.wayhint()` now reports `aim`, `walked`, `crow`, `routes` and
`drawnAt`.

**AND THE EXECUTOR MOVES WITH IT** (`tools/playtest/adapter_emberbrook.mjs`). Round 10's rule
is *"a pixel on an arrow resolves to the place the arrow points at"*; the aim changes that
place, so the aimed marker publishes it as `data-way-at` and `markerAt` reads it. Without
that half the fix would be invisible to the harness AND wrong for it: the agent would keep
driving the straight line the aim exists to refuse.

### §4 MEASURED, on the engine's own projection (`wayfind_probe`, real Chrome)

`docs/qa/playtest/wayfind-r28-cottage`, `-r28-moorage` — the frames are pinned.

| station | seam draws at | arrow NOW draws at | `SIM.pick` at the arrow's own pixel |
|---|---|---|---|
| PT-044 moorage, `--from ch2.supper` | `[713, 291]` | `[644, 450]` — aim `[73.8, 2.62, −27.3]` | `walk_e_weave-huts__moorage_l2_t00` |
| PT-045 cottage door, `--from ch2.winches` | `[568, 686]` | `[393, 576]` — aim `[91.63, 8.3, −21.5]` | `walk_e_lockhead__keepers-cottage_l16` |
| PT-045 spur mid `[89.41, 9.33, −20.84]` | `[568, 686]` | `[393, 572]` — same aim | `lf_planking` |
| PT-044 mid-switchback `[72.6, 3.3, −26.9]` | `[713, 291]` | `[715, 270]` — **aim null** | `t2c_W2_laundry_deckB_3` |

The aim at the cottage is `[91.63, 8.3, −21.5]`: **the east detour `_court_probe --way` had
already driven 4/4 both ways.** The aim at the moorage resolves to a tread of the switchback's
lowest flight — the climb round 27 called "5 m west, unmarked". And the fourth row is the
control: from the hairpin the straight line is honest, the aim withdraws by itself, and the
arrow is round 19's clamp again. **A body standing on the tenant-shack boardwalk at
`[74, 1.3, −27.5]` is aimed BACK EAST to `[76.09, 1, −27]`** — the stair foot — which is the
route being honest about a stand that is past the flight, not a bug.

The frames were looked at, not only measured: at the moorage the pill sits on the plank deck
