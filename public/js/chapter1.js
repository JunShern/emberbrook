...
const Chapter1 = {
  ...
  interact(p) {
    ...
    if (t.kind === 'waystone') {
      Dialog.start([{ who: 'system', text: p.role === 'vesper'
        ? 'The waystone. It matches the sketch in her book, line for line — even the crack through the chin.'
        : 'An old waystone marking the south road. The carved face is worn nearly smooth.' }]);
    }
    if (t.kind === 'hearth') {
      Dialog.start([
  ...
  lightLamp(lamp, p) {
    ...
    if (this.flags.lampsLit === 3)
      Dialog.start([
        ...
        { who: 'lake', text: '(A lit lamp near every door. Now the Hour can start.)' },
      ]);
  },
  ...
  talkTo(key, ent, p) {
    ...
    if (!F.hushDone) {
      if (key === 'rowan') {
        if (isVesper) {
          ...
          return D([
            ['rowan:happy', 'Now then! A guest, and on Emberwake of all nights. Welcome, my dear.'],
            ['vesper', 'Vesper. Mapmaker. I need to get through your gate, and I’m told you’re in charge.'],
            ['rowan:happy', 'In charge! Ha! I keep the ledger; the village keeps itself.'],
            ...
            ['rowan:happy', '…Ha! I like you. Stay the night, then — and stay for the Kindling Hour.'],
            ...
            ['rowan:happy', 'Three hundred years that flame has burned. In all of them, nobody here carried a grief alone.'],
            ['rowan:happy', 'That is not boasting, my dear. It is bookkeeping — I checked.'],
            ...
            ['rowan:grave', 'As for the Gate — lamplighter business, child. His order built it, his order shut it.'],
            ...
            ['vesper:thinking', 'Apologizing to lamps.'],
      ...
      if (key === 'poppy') {
        ...
        return D([
          ...
          ['lake', '…Half a bun, then. Thanks.'],
          ['poppy', 'No use asking if YOU’LL make a telling this year. Keeper keeps his own — I know, I know.'],
          ['poppy', 'Strangest rule your family ever kept, love. Saddest one, too.'],
          ['lake', 'Somebody has to. I don’t mind it, most nights.'],
      ...
      if (key === 'mara' || key === 'pip') {
        if (isVesper) { F.vesperTalked.mara = true;
          return D([
            ...
            ['mara', 'She is not a moon.'],
            ['pip', 'Are you a REAL mapmaker? Have you been everywhere?'],
            ['vesper', 'Most places. Not everywhere.'],
            ['pip', 'Have you been to the MOON?'],
            ['vesper', '…Not yet.'],
            ['pip', 'She’s been to the moon.'],
            ['mara', 'He’s been up since dawn for tonight. He’ll fall asleep mid-sentence and deny it mid-fall.'],
            ['mara', 'He’ll remember tonight his whole life. Some grown-up Emberwake, he’ll tell it to the flame.'],
            ['vesper', 'That’s a nice thought. I mean that plainly, for once.'],
          ]);
        }
        return D([
          ...
          ['lake', 'Evening, Pip. That’s a good stick.'],
          ['pip', 'It’s the BEST stick. You can hold it for one minute.'],
          ['lake', 'After the rounds. A stick like that deserves both hands.'],
          ['pip', 'Renn says you can SEE the memories go in. I’m staying awake to check.'],
          ['lake', 'Renn’s right. You can.'],
    ...
    if (key === 'poppy') {
      if (!F.seen.poppy) { F.seen.poppy = true;
        return D([
          ...
          ['poppy:hollow', '…Do I do it anyway?'],
          ['lake:tender', 'Every morning.'],
    ...
    if (key === 'finn') {
      if (!F.seen.finn) { F.seen.finn = true;
        return D([
          ...
          ['finn:happy', '…Finn. Huh. Short. I like it.'],
    ...
    if (key === 'mara' || key === 'pip') {
      if (!F.seen.mara) { F.seen.mara = true;
        return D([
          ['pip:scared', 'Tell her. TELL her!'],
          ['lake:determined', 'Mara. This is Pip. Your son, seven years old.'],
          ['lake:tender', 'You waited out a snowstorm at the pass for him to be born.'],
          ...
          ['vesper:worried', 'Pip. Look at me. I saw her holding your hand — an hour ago, by the stall.'],
  ...
  playVesperIntro(vesper) {
    ...
    Cutscene.play([
      { banner: { title: '— VESPER —', sub: 'a mapmaker, a long way from home', dur: 5 } },
      { cam: { x: 300, y: 620, viewH: 460 } },
      { wait: 1.2 },
      { narrate: 'On the last night of autumn, a mapmaker walked into a valley that was not on any of her maps.' },
      { camRelease: true },
    ]);
  },
  ...
  playWaystone(vesper) {
    ...
    Cutscene.play([
      ...
      { wait: 0.6 },
      { say: ['mochi', 'Mrrp.'] },
      { say: ['vesper', 'GAH— …a cat. Hello. Don’t sneak up on people in haunted forests.'] },
      { say: ['mochi', 'Mrrp.'] },
      { say: ['vesper', 'There are lights up the road. Come on. If anyone can explain my sketchbook, it’s whoever lives here.'] },
  ...
  playLakeIntro(lake) {
    ...
    Cutscene.play([
      ...
      { say: ['lake:tender', '(A year tonight since she set the lighter down. I miss her most at rounds-time.)'] },
  ...
  playMeet(players) {
    ...
    Cutscene.play([
      ...
      { say: ['rowan:happy', 'The HOUR! Gather, neighbors — to the square!'] },
      { say: ['rowan:happy', 'Lake — the flame.'] },
      { camRelease: true },
  ...
  playKindlingHour(players) {
    ...
    Cutscene.play([
      ...
      { say: ['rowan:happy', 'Three hundred years, and the warmth of every one of them — right here.'] },
      ...
      { say: ['rowan:happy', 'Who brings the first memory?'] },
      ...
      { say: ['pip:happy', 'This is the BEST night.'] },
      { wait: 1.0 },
      ...
      { say: ['lake:determined', 'Every window in this village. Every name behind it.'] },
      ...
      { say: ['vesper:worried', 'I got here an HOUR ago.'] },
      ...
      { say: ['rowan:grave', 'See to them — all of them. They deserve their names back, even borrowed.'] },
      { say: ['rowan:hollow', 'Then come find me. I will be with my ledger — every word of it true, and not one of them mine.'] },
  ...
  playPact(p) {
    ...
    Cutscene.play([
      ...
      { say: ['rowan:grave', 'Ink keeps facts. It cannot keep what they WEIGHED.'] },
      ...
      { say: ['rowan:grave', 'And the old Order kept one creed above all: light does not die — it is only ever carried.'] },
      ...
      { say: ['vesper:worried', 'Since I was six years old, I have drawn one clearing. Over and over, in dreams.'] },
      { say: ['vesper:worried', 'Forty-one drawings of the same clearing.'] },
      { say: ['vesper:determined', 'I came here because the forty-first had YOUR gate in the corner.'] },
      ...
      { say: ['rowan:grave', 'So it must be both of you, together.'] },
      ...
      { say: ['rowan:grave', 'The old rite, then. Two keepers, one flame — say it and mean it, or the Gate will know.'] },
      { say: ['rowan:grave', '“What they forgot, we keep. What we keep, we return.”'] },
      ...
      { say: ['rowan:grave', 'Twin sigils, before the Gate. The Lamplighters cut them, in the old days.'] },
      { say: ['rowan:grave', 'That road was always meant to be walked by two.'] },
    ]);
  },
  ...
  playEnding(players) {
    ...
    Cutscene.play([
      ...
      { say: ['vesper:tender', 'For the record: this is now the best chart I carry.'] },
      ...
      { say: ['lake:determined', 'My grandmother had a saying for the rounds. The first lamp is the whole job — the rest is only more of it.'] },
      { say: ['vesper', 'Is that supposed to help?'] },
      { say: ['lake', '…It always did. A little.'] },
      { say: ['vesper:happy', 'Then it goes in the book. Come on, partner — first lamp.'] },
